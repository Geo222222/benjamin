...
class BenjaminBookSigner:
    """Producer-side signer for Benjamin-owned Book Evidence Protocol v2 events.
    ...
    @classmethod
    def from_private_key_b64(cls, *, key_id: str, private_key_b64: str) -> "BenjaminBookSigner":
        try:
    ...
    def sign_v2_envelope(
        ...
        *,
        ...
        body: dict[str, object] = {
            ...
            "payload_digest": _require_digest(payload_digest),
...
def load_benjamin_book_signer_from_env(env: Mapping[str, str] | None = None) -> BenjaminBookSigner:
    source = os.environ if env is None else env
    key_id = source.get("BENJAMIN_BOOK_KEY_ID", "")
    private_key_b64 = source.get("BENJAMIN_BOOK_ED25519_PRIVATE_KEY_B64", "")
    if not key_id or not private_key_b64:
        raise BookBridgeError(
            "Benjamin Book signing is unavailable: BENJAMIN_BOOK_KEY_ID and "
            "BENJAMIN_BOOK_ED25519_PRIVATE_KEY_B64 are required"
        )
    return BenjaminBookSigner.from_private_key_b64(key_id=key_id, private_key_b64=private_key_b64)
